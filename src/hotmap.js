...
export default class Hotmap {
  constructor(params) {
    ...
    if (!this.hideYMeta) {
      this.rowCatColors = this.yMeta ? categoryColors(this.yMeta) : [];
    }
  ...
  static validateParams(params) {
    ...
    if (!ele) alert(`${NAME}: Must provide an element to attach chart to.`);
      ...
      alert(`${NAME}: Must provide an matrix of values.`);

    // validate data
    let validMat =
      matrix.filter((r) => r.length !== matrix[0].length).length == 0;
    if (!validMat)
      alert('Must provide matrix with same number of elements in each row.');
  }
  ...
  init() {
    ...
    if (this.opts.hideOptions) this.ele.querySelector('.opts-btn').remove();
    ...
    // dimensions for meta
    this.yMetaWidth =
      this.yMeta && !this.hideYMeta ? this.yMeta[0].length * yMetaSize : 0;
    this.xMetaHeight =
      this.xMeta && !this.hideXMeta ? this.xMeta[0].length * xMetaSize : 0;
  ...
  async start() {
    ...
    // initialize options
    if (!this.opts.hideOptions) this.options = this.initOptions();

    // optional tree (experimental)
    if (this.newick) {
      const Tree = await import(
        /* webpackChunkName: "hotmap-tree" */ './tree.js'
      );
  ...
  initStage(firstInit) {
    this._draw(false, false, false, firstInit);
  }
  ...
  initChart(firstInit) {
    ...
    this.ele.querySelector('.webgl-canvas').appendChild(this.renderer.view);
   ...
   */
  _draw(renderX, renderY, scale, firstInit) {
  ...
  drawYLabel(svgEl, text, x, y, cellIdx) {
    ...
    fontSize =
      fontSize <= this.opts.maxFontSize ? fontSize : this.opts.maxFontSize;
    ...
    ele.innerHTML = this.shortenLabel(
      fontSize,
      text,
      yTextSpace,
      this.yLabelEllipsisPos
    );
  ...
  highlightQuery() {
    ...
    colMatches.forEach((isMatch, i) => {
      ...
      if (this.cellW > minTextW) {
        this.highlightLabel(
          this.query.q,
          this.xAxis.querySelector(`[data-i="${i}"]`),
          i
        );
      }
    });
  ...
  highlightLabel(text, ele, i) {
    ...
    if (idx === -1) {
      ...
      ele.innerHTML = label
        .replace(
          overlap,
          `<tspan font-weight='bold' fill='#1187f1'>${overlap}</tspan>`
        )
        .replace('...', `<tspan font-weight='bold' fill='#1187f1'>...</tspan>`);
      return;
    }
  ...
  showYAxisLabel(label) {
    ...
    if (ele) {
      ele.innerHTML = label;
      let y =
        margin.top + ele.getBBox().width / 2 + (yViewSize * this.cellH) / 2;
      ele.setAttribute('y', y);
      ele.setAttribute('transform', `rotate(-90, ${x}, ${y})`);
      return;
    }
  ...
  addCategoryLabel(axis, text, x, y, idx) {
    if (this.isTransposed) {
      console.warn(
        `${NAME}: flipAxis does not (currently) support categories.`
      );
      return;
    }
  ...
  initScrollBox() {
    return new ScrollBox({
      ...
      onMouseWheel: (change) => {
        ...
        this.cellW =
          newXDim < cellMin ? cellMin : newXDim > cellMax ? cellMax : newXDim;
  ...
  initSearch() {
    ...
    searchInput.onkeyup = function() {
      ...
      if (!q.length) {
        ...
        self.query = {
          q: q,
          count: self.cols.filter((c) => c.name.toLowerCase().includes(q))
            .length,
        };
      }
  ...
  tooltip(content, top, left, offset = { w: 0, h: 0 }) {
    ...
    top =
      top + h + tooltip.offsetHeight < height
        ? top + h
  ...
  updateLegend() {
    this.ele.querySelector('.legend').innerHTML = legend(
      this.size.min,
      this.size.max,
      this.color
    );
    ...
    if (typeof Picker !== 'undefined') this.updateColorPicker();
  }
  ...
  yAxisHover() {
    ...
    axisBox.onmousemove = (evt) => {
      ...
      dragBox.innerHTML =
        `<div class="y-drag-icon-container" style="margin-top: ${top}px;">` +
        icon.innerHTML +
        `</div>` +
  ...
  xAxisHover() {
    ...
    this.ele.querySelector('.header').onmouseenter = () =>
      this.hideHoverEffects();
  }
  ...
  yMousePosToRowIdx(evt) {
    let y =
      evt.pageY -
      (this.opts.useBoundingClient
        ? this.ele.getBoundingClientRect().y
        : this.ele.offsetTop);
  ...
  xMousePosToColIdx(evt) {
    let x =
      evt.pageX -
      (this.opts.useBoundingClient
        ? this.ele.getBoundingClientRect().x
        : this.ele.offsetLeft);
  ...
  getCol(j) {
    ...
    const selection = this.matrix.map((val, i) => {
      ...
      return Object.assign(
        { name: this.rows[i].name, val: val[j] },
        { colID, rowID }
      );
    });
  ...
  selectable() {
    ...
    let selectDraw = () => {
      ...
      let w =
        box.w < this.cellW ? (box.w + 1) * this.cellW : box.w * this.cellW;
      let h =
        box.h < this.cellH ? (box.h + 1) * this.cellH : box.h * this.cellH;
  ...
  getMatrixStats(matrix) {
    if (this.defaults.size) {
      const defaultSize = this.defaults.size;
      const result = {
        m: defaultSize.rows,
        n: defaultSize.columns,
        size: defaultSize.rows * defaultSize.columns,
        min: defaultSize.min,
        max: defaultSize.max,
      };
      return result;
  ...
  snapshot(fullChart) {
    ...
    let { svg, xAxis, yAxis, cAxis } = this.initSVGContainers(
      div,
      width,
      height
    );
    ...
    for (let i = 0; i < ySize; i++) {
      ...
      if (cellH >= minTextW && !this.tree) {
        this.drawYLabel(
          yAxis,
          this.rows[rowIdx].name,
          margin.left - this.yMetaWidth - 10,
          y,
          i
        );
      }
      ...
      if (this.yMeta && rowIdx < this.size.m) {
        ...
        for (let i = 0; i < categories.length; i++) {
          ...
          let rect = svgRect(x, y, width - 1, cellH, {
            fill: hexToHexColor(color),
          });
  ...
  updateMatrix(matrix) {
    ...
    if (this.cells.children.length <= 0) {
      this.initChart(true);
    } else {
      let canvas = this.ele.querySelector('.webgl-canvas canvas');
      if (canvas) canvas.remove();
